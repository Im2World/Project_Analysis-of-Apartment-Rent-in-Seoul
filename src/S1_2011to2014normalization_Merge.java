package SeoulAptPrj;
//1. 2011~2014년 4개 csv파일을 형식 가공 후 1개 파일로 통합
/*
**알고리즘**
1) 한 디렉터리안에서 전체 파일 대상 반복작업
2) split =>  가나,다라,"35,000",마바사 => 이러한 데이터를 분할시 4개가 아닌 3개 열로 분할하도록 정규식 사용
3) 문제가 되는 "35,000" 처리
...
public class S1_2011to2014normalization_Merge {
	public void S1_2011to2014normalization_Merge() throws IOException {
		...
		File saveFile = new File("C:\\Users\\IM\\Desktop\\javaBasicPrj\\aptDoroPrj\\tmp\\2011to2014.csv");	// 해당 파일로 저장할 것.
		...
		while ((readtxt = br.readLine()) != null) { //br을 한줄 씩 읽어 readtxt에 저장 
			...
			if (line > 15) {
				if (field[9].contains(",")) { // "35,000"
					field[9] = field[9].replaceAll(",","");   // ,제거 => "35000"
